...
const MENU_ITEMS = [
  ...
  { key: 'PostJob',       label: 'Post a Job',      icon: 'add-circle-outline',    badge: null },
  { key: 'Mypostings',    label: 'My Postings',     icon: 'document-text-outline', badge: null },
  { key: 'Sentoffers',    label: 'Sent Offers',     icon: 'paper-plane-outline',   badge: null },
  { key: 'Hiredtalents',  label: 'Hired Talent',    icon: 'people-outline',        badge: null },
];

const COMM_ITEMS = [
  { key: 'Message',   label: 'Messages',   icon: 'chatbubble-outline', badge: null },
  { key: 'ClientProfile', label: 'My Profile', icon: 'person-outline',     badge: null },
...
export default function ClientScreen({ onNavigate }) {
  ...
  const { clientJobs, isLoading: jobsLoading } = useSelector((state) => state.jobs.jobs);
  const { sentOffers, stats: offerStats, isLoading: offersLoading } = useSelector((state) => state.offers);
  ...
  useEffect(() => {
    ...
    // Add recent offers sent
    if (sentOffers && sentOffers.length > 0) {
      const recentOffers = sentOffers.slice(0, 3);
      recentOffers.forEach(offer => {
        ...
        activities.push({
          ...
          title: `Offer ${offer.status === 'pending' ? 'sent to' : offer.status === 'accepted' ? 'accepted by' : 'declined by'} ${offer.freelancer_name}`,
          sub: `${offer.job_title} · ₱${offer.amount?.toLocaleString()}`,
  ...
  const activePostings = clientJobs?.filter(job => job.status === 'open').length || 0;
  const pendingOffers = sentOffers?.filter(offer => offer.status === 'pending').length || 0;
  const acceptedOffers = sentOffers?.filter(offer => offer.status === 'accepted').length || 0;
  const hiredCount = acceptedOffers;
  const totalSpent = offerStats?.totalSpent || 0;
  ...
  const SidebarItem = ({ item }) => {
    ...
    let badgeValue = null;
    if (item.key === 'Sentoffers' && pendingOffers > 0) {
        ...
        <View style={styles.main}>
          ...
          <View style={styles.topbar}>
            ...
            <Text style={styles.topbarTitle}>
              Client <Text style={styles.topbarTitleItalic}>Dashboard</Text>
            </Text>
              ...
              <>
                <View style={styles.statsGrid}>
                  ...
                  >
                    ...
                    <Text style={styles.statSub}>Open jobs</Text>
                  </TouchableOpacity>
                  ...
                  >
                    <View style={styles.statTop}>
                      <Text style={styles.statLabel}>SENT OFFERS</Text>
                      <Ionicons name="paper-plane-outline" size={14} color="rgba(255,255,255,0.3)" />
                    </View>
                    <Text style={styles.statNumber}>{sentOffers?.length || 0}</Text>
                    {pendingOffers > 0 && (
                      <Text style={[styles.statSub, { color: GOLD }]}>{pendingOffers} pending reply</Text>
                    )}
                  </TouchableOpacity>

                  <TouchableOpacity 
                    style={styles.statCard}
                    onPress={() => onNavigate('Hiredtalents')}
                    activeOpacity={0.7}
                  >
                    <View style={styles.statTop}>
                      <Text style={styles.statLabel}>HIRED TALENT</Text>
                      <Ionicons name="people-outline" size={14} color="rgba(255,255,255,0.3)" />
                    </View>
                    <Text style={styles.statNumber}>{hiredCount}</Text>
                    <Text style={styles.statSub}>Accepted offers</Text>
                  </TouchableOpacity>
                  ...
                  <View style={styles.statCard}>
                    ...
                    <Text style={[styles.statSub, { color: GOLD }]}>Total project cost</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  postJobBtn: { paddingHorizontal: 12, paddingVertical: 8, backgroundColor: GOLD, borderRadius: 10 },
  postJobText: { fontSize: 11, fontWeight: '600', color: '#0a0a0a' },
