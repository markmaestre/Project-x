// components/RatingFreelancer.jsx - For CLIENTS to rate FREELANCERS
...
import { rateFreelancer, checkClientCanRate } from '../../Redux/slices/ratingSlice';
...
const RatingFreelancer = ({ 
  ...
  // Check if client can rate this freelancer
  useEffect(() => {
    if (visible && jobId) {
      checkCanRate();
    }
  ...
  const checkCanRate = async () => {
    setChecking(true);
    try {
      const result = await dispatch(checkClientCanRate(jobId)).unwrap();
      setCanRate(result.canRate);
      if (!result.canRate) {
        Alert.alert('Info', result.message || 'You cannot rate this freelancer');
        if (result.hasRated) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await dispatch(rateFreelancer({
      ...
      Alert.alert('Success', `You rated ${freelancerName || 'the freelancer'} successfully!`);
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.infoContainer}>
              ...
              <Text style={styles.infoText}>
                You have already rated this freelancer or the job is not yet completed.
              </Text>
  ...
  return (
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <View style={styles.header}>
            <Text style={styles.title}>Rate Freelancer</Text>
            <TouchableOpacity onPress={handleClose} style={styles.closeBtn}>
              <Ionicons name="close" size={24} color={TEXT_MUTED} />
            </TouchableOpacity>
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.jobInfo}>
              ...
              <Text style={styles.recipientLabel}>Freelancer</Text>
              <Text style={styles.recipientName}>{freelancerName || 'Freelancer'}</Text>
            </View>
            ...
            <View style={styles.detailedSection}>
              ...
              {renderCategoryRating('Quality of Work', quality, setQuality)}
              {renderCategoryRating('Professionalism', professionalism, setProfessionalism)}
              {renderCategoryRating('On Time Delivery', onTime, setOnTime)}
            </View>
