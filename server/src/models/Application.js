...
export default function MyPostings({ onNavigate }) {
  ...
  // Render Education Section - FROM APPLICATION DATA
  const renderEducation = (education) => {
    if (!education || !education.level) return (
      <View style={styles.detailSection}>
        <Text style={styles.detailSectionTitle}>🎓 Education</Text>
        <View style={styles.detailCard}>
          <Text style={styles.detailValue}>No education information provided</Text>
        </View>
      </View>
    );
  ...
  // Render Experience Section - FROM APPLICATION DATA
  const renderExperience = (experiences) => {
    if (!experiences || experiences.length === 0) return (
      <View style={styles.detailSection}>
        <Text style={styles.detailSectionTitle}>💼 Work Experience</Text>
        <View style={styles.detailCard}>
          <Text style={styles.detailValue}>No work experience provided</Text>
        </View>
      </View>
    );
        ...
        {experiences.map((exp, index) => (
          <View key={index} style={styles.detailCard}>
            ...
            <Text style={styles.detailValue}>
              {exp.start_date || 'Start date not specified'} - {exp.currently_working ? 'Present' : (exp.end_date || 'End date not specified')}
            </Text>
  ...
  const renderResume = (resume) => {
    if (!resume || !resume.url) return (
      <View style={styles.detailSection}>
        ...
        <View style={styles.detailCard}>
          <Text style={styles.detailValue}>No resume uploaded</Text>
        </View>
      </View>
    );
  ...
  // Render Skills Section - FROM FREELANCER PROFILE
        ...
        <View style={[styles.modalContent, { maxHeight: '90%' }]}>
          ...
          {selectedApplicant && selectedApplication && (
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.detailCard}>
                  ...
                  {selectedApplicant.city && selectedApplicant.country && (
                    <>
                      ...
                      <Text style={styles.detailValue}>{selectedApplicant.city}, {selectedApplicant.country}</Text>
                    </>
                  )}
                  {selectedApplicant.address && (
                    <>
                      <Text style={styles.detailLabel}>Address:</Text>
                      <Text style={styles.detailValue}>{selectedApplicant.address}</Text>
                    </>
                  )}
                </View>
              </View>

              {/* Skills - FROM FREELANCER PROFILE */}
              {renderSkills(selectedApplicant.skills)}

              {/* About/Bio - FROM FREELANCER PROFILE */}
              ...
              {/* RESUME - FROM APPLICATION DATA */}
              {renderResume(selectedApplication.resume)}

              {/* EDUCATION - FROM APPLICATION DATA (ITO ANG TAMA) */}
              {renderEducation(selectedApplication.education)}

              {/* EXPERIENCE - FROM APPLICATION DATA (ITO ANG TAMA) */}
              {renderExperience(selectedApplication.experiences)}

              {/* Cover Letter - FROM APPLICATION DATA */}
              <View style={styles.detailSection}>
              ...
              {/* Proposed Rate - FROM APPLICATION DATA */}
              {selectedApplication.proposed_rate && (
                <View style={styles.detailSection}>
                  <Text style={styles.detailSectionTitle}>💰 Proposed Rate</Text>
                  <View style={styles.detailCard}>
